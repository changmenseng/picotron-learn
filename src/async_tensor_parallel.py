...
class AsyncAllGatherLinear(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input_shard, weight, bias, process_group_manager):
        """
            Args:
                input_shard: (B, L, D_i / N)
                weight: (D_o / N, D_i)
                bias: (D_o / N)
            Output:
                hidden: (B, L, D_o / N)
        ...
        input_shard_size = input_shard.shape[-1]
        assert input_shard_size * process_group_manager.tp_world_size == input_size
        ctx.process_group_manager = process_group_manager

        if ctx.process_group_manager.tp_world_size == 1:
            return F.linear(input_shard, weight, bias)
        

        src_rank = (process_group_manager.tp_world_size + process_group_manager.tp_rank - 1) % process_group_manager.tp_world_size
        dst_rank = (process_group_manager.tp_rank + 1) % process_group_manager.tp_world_size
        input_shards = [input_shard if i == process_group_manager.tp_rank else torch.empty_like(input_shard, requires_grad=True) for i in range(process_group_manager.tp_rank)]
        output = 0
        for i in range(process_group_manager.tp_world_size):
            send_input_shard_id = (process_group_manager.tp_world_size + process_group_manager.tp_rank - i) % process_group_manager.tp_world_size
            recv_input_shard_id = (process_group_manager.tp_world_size + process_group_manager.tp_rank - i - 1) % process_group_manager.tp_world_size
            if i != process_group_manager.tp_world_size - 1:
                reqs = dist.batch_isend_irecv(
                    [
                        dist.P2POp(dist.isend, input_shards[send_input_shard_id], dst_rank),
                        dist.P2POp(dist.irecv, input_shards[recv_input_shard_id], src_rank)
                    ]
                )
            output_shard = F.linear(
                input=input_shards[send_input_shard_id], 
                weight=weight[
                    :, 
                    input_shard_size * send_input_shard_id:
                    input_shard_size * (send_input_shard_id + 1)
                ], 
                bias=None
            )
            output = output + output_shard # (B, L, D_o / N)
            if i != process_group_manager.tp_world_size - 1:
        ...
        ctx.save_for_backward(input_shard, weight, bias, output)

        return output
    
    @staticmethod
    def backward(ctx, grad_output):
        input_shard, weight, bias, output = ctx.saved_tensors
        torch.autograd.backward(output, grad_output)
        # reduce scatter
        return input_shard.grad, weight.grad, bias.grad if bias is not None else None

class AsyncReduceScatterLinear(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input_shard, weight, bias, process_group_manager):
        """
            Args:
        ...
        assert output_size % process_group_manager.tp_world_size == 0
        output_shard_size = output_size // process_group_manager.tp_world_size
        ctx.process_group_manager = process_group_manager
        ...
        output_shard = torch.zeros(
            *input_shard.shape[:-1], output_shard_size, 
            dtype=input_shard.dtype, 
            device=input_shard.device
        ) # (B, L, D_o / N)
        recv_output_shard = torch.zero_like(output_shard)
        bias = bias / process_group_manager.tp_world_size if bias is not None else None
        for i in range(process_group_manager.tp_world_size):
            if i != 0:
                [req.wait() for req in reqs]
            output_shard_id = (process_group_manager.tp_world_size - process_group_manager.tp_rank - 1 + i) % process_group_manager.tp_world_size
            output_shard = F.linear(
                input=input_shard, 
                weight=weight[
                    output_shard_size * output_shard_id:
                    output_shard_size * (output_shard_id + 1),
                    :
                ],
                bias=bias[
                    output_shard_size * output_shard_id:
            ...
            output_shard = recv_output_shard + output_shard
            if i != process_group_manager.tp_world_size - 1:
                reqs = dist.batch_isend_irecv(
                    [
                        dist.P2POp(dist.isend, output_shard, dst_rank),
                        dist.P2POp(dist.irecv, recv_output_shard, src_rank)
                    ]
                )
...
class ColumnParallelLinear(nn.Linear):
    ...
    def forward(self, input_shard):
        if not self.distributed_inited:
            raise Exception("Not init yet! Please call .distributed_init method first.")
        
        src_rank = (self.process_group_manager.tp_world_size + self.process_group_manager.tp_rank - 1) % self.process_group_manager.tp_world_size
        dst_rank = (self.process_group_manager.tp_rank + 1) % self.process_group_manager.tp_world_size
        
        input_shards = [input_shard if i == self.process_group_manager.tp_rank else torch.zero_like(input_shard) for i in range(self.process_group_manager.tp_rank)]
        output = 0
        for i in range(self.process_group_manager.tp_world_size - 1):
            send_input_shard_id = (self.process_group_manager.tp_rank + i) % self.process_group_manager.tp_world_size
            recv_input_shard_id = (self.process_group_manager.tp_rank + i + 1) % self.process_group_manager.tp_world_size
            send_op = dist.P2POp(dist.isend, input_shards[send_input_shard_id], dst_rank)
            recv_op = dist.P2POp(dist.irecv, input_shards[recv_input_shard_id], src_rank)
            reqs = dist.batch_isend_irecv([send_op, recv_op])
            output_shard = F.linear(input_shards[send_input_shard_id], self.weight, self.bias)
            output += output_shard
            [req.wait() for req in reqs]
